...
interface SentioChatRecordState {
    ...
    updateRecord: (message: ChatMessage) => void;
}
export const useChatRecordStore = create<SentioChatRecordState>()(
    persist(
        (set) => ({
            chatRecord: [],
            addChatRecord: (message: ChatMessage) =>
                set((state) => ({
                    chatRecord: [...state.chatRecord, message],
                })),
            getLastRecord: () => {
                const chatRecord: ChatMessage[] =
                    useChatRecordStore.getState().chatRecord;
                return chatRecord.length > 0
                    ? chatRecord[chatRecord.length - 1]
                    : undefined;
            },
            updateLastRecord: (message: ChatMessage) =>
                set((state) => {
                    const newRecord = [
                        ...state.chatRecord.slice(0, -1),
                        message,
                    ];
                    // 防止重复的AI回复
                    if (newRecord.length >= 2) {
                        ...
                        if (
                            lastTwo[0].role === CHAT_ROLE.AI &&
                            lastTwo[1].role === CHAT_ROLE.AI &&
            ...
            deleteLastRecord: () =>
                set((state) => ({
                    chatRecord: [...state.chatRecord.slice(0, -1)],
                })),
            clearChatRecord: () => set((state) => ({ chatRecord: [] })),
            updateRecord: (message: ChatMessage) =>
                set((state) => {
                    return {
                        chatRecord: state.chatRecord.map((record) =>
...
export const useSentioBasicStore = create<SentioBasicState>()(
    persist(
        (set) => ({
            ...
            setShowThink: (showThink: boolean) =>
                set((state) => ({ showThink: showThink })),
            setLipFactor: (weight: number) =>
                set((state) => ({ lipFactor: weight })),
        }),
        {
    ...
    persist(
        (set) => ({
            ...
            setEnable: (enable: boolean) =>
                set((state) => ({ enable: enable })),
            setInferType: (infer_type: IFER_TYPE) =>
                set((state) => ({ infer_type: infer_type })),
            setEngine: (by: string) => set((state) => ({ engine: by })),
            setSettings: (by: { [key: string]: any }) =>
                set((state) => ({ settings: by })),
        }),
        {
...
export const useSentioBackgroundStore = create<SentioBackgroundState>()(
    persist(
        (set) => ({
            ...
            setBackground: (by: ResourceModel | null) =>
                set((state) => ({ background: by })),
        }),
        {
...
export const useSentioCharacterStore = create<SentioCharacterState>()(
    persist(
        (set) => ({
            character: null,
            setCharacter: (by: ResourceModel | null) =>
                set((state) => ({ character: by })),
        }),
        {
...
export const useSentioLive2DStore = create<SentioLive2DState>()((set) => ({
    ready: false,
