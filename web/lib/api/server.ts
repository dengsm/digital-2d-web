...
export async function api_asr_get_list(): Promise<PROTOCOL.EngineDesc[]> {
    ...
    return get(path, null)
        .then((response: PROTOCOL.EngineListResponse) => {
...
export async function api_asr_get_default(): Promise<PROTOCOL.EngineDesc> {
    ...
    return get(path, null)
        .then((response: PROTOCOL.EngineDefaultResponse) => {
            return response.data;
        })
        .catch(() => {
            return {} as PROTOCOL.EngineDesc;
...
export async function api_asr_get_config(
    engine: string,
): Promise<PROTOCOL.EngineParamDesc[]> {
    const path = `${ASR_PATH}/engine/${engine}`;
    return get(path, null)
        .then((response: PROTOCOL.EngineConfigResponse) => {
            return response.data;
        })
        .catch(() => {
...
export async function api_asr_infer(
    ...
    return post(path, body, null)
        .then((response: PROTOCOL.StringResponse) => {
            return response.data;
        })
        .catch(() => {
            return "";
...
export async function api_asr_infer_file(
    ...
    return filePost(path, formData, null)
        .then((response: PROTOCOL.StringResponse) => {
            return response.data;
        })
        .catch(() => {
...
export async function api_tts_get_voice(
    ...
    return get(path, null)
        .then((response: PROTOCOL.VoiceListResponse) => {
            return response.data;
        })
        .catch(() => {
...
export async function api_tts_infer(
    ...
    return post(path, body, signal)
        .then((response: PROTOCOL.BaseResponse) => {
            return response.data;
        })
        .catch(() => {
            return "";
...
export function api_agent_stream(
    ...
    signal: AbortSignal,
    onOk: (response: PROTOCOL.EventResponse, conversation_id: string) => void,
    onError: (error: Error, conversation_id: string) => void = (error) => {},
    imageData?: string,
) {
    // console.log(
    //     "API 发送请求，图片数据:",
    //     imageData ? `长度: ${imageData.length}` : "无图片数据",
    // );
    const path = `${AGENT_PATH}/engine`;
    ...
    fetchEventSource(url, {
        ...
        onmessage: (msg) => {
            ...
            onOk(eventResp, conversation_id);
        },
        onerror(error) {
            throw new Error(error);
        },
    }).catch((error) => {
        onError(error, conversation_id);
        errorHandler(error, signal);
    });
}

// =========================== Custom APIs ===========================
export async function api_get_engine_list(engineType: string) {
    switch (engineType) {
...
export async function api_get_engine_default(engineType: string) {
    switch (engineType) {
        case PROTOCOL.ENGINE_TYPE.ASR:
...
export function api_get_engine_config(engineType: string, engine: string) {
    switch (engineType) {
        case PROTOCOL.ENGINE_TYPE.ASR:
