...
import {
    SENTIO_TTS_PUNC,
    SENTIO_TTS_SENTENCE_LENGTH_MIN,
} from "@/lib/constants";
...
export const ChatInput = memo(({ postProcess }: IProps) => {
    ...
    const { addChatRecord, updateLastRecord, updateRecord } =
        useChatRecordStore();
    const { engine: agentEngine, settings: agentSettings } =
        useSentioAgentStore();
    ...
    const handleStopRecord = async () => {
        try {
            ...
            try {
                ...
                // 进行语音识别
                const asrResult = await api_asr_infer_file(
                    asrEngine,
                    asrSettings,
                    mp3Blob,
                );
    ...
    const onSendClick = async () => {
        ...
        sendMessageWithScreenshot(currentMessage, ai_message_id);
    };
    ...
    const sendMessageWithScreenshot = async (
        ...
        sendMessageToBackend(messageContent, screenshotDataUrl, message_id);
    };
    ...
    const sendMessageToBackend = (
            ...
            if (!agentDone || accumulatedResponse.length > ttsProcessIndex) {
                ...
                while (beginIndex >= ttsProcessIndex) {
                    ...
                    if (puncIndex > beginIndex) {
                        if (
                            puncIndex - ttsProcessIndex >
                            SENTIO_TTS_SENTENCE_LENGTH_MIN
                        ) {
                            ttsText = accumulatedResponse.substring(
                                ttsProcessIndex,
                                puncIndex + 1,
                            );
        ...
        const agentCallback = (response: any, conversation_id: string) => {
            // console.log(conversation_id, response);
        ...
        // 定义错误回调函数处理AI响应错误
        const agentErrorCallback = (error: Error, conversation_id: string) => {
    ...
    return (
        <div className="flex flex-col w-4/5 md:w-2/3 2xl:w-1/2 items-start z-10 gap-2">
            <div className="flex w-full items-center z-10 space-x-2">
                <Input
...
export const ChatStreamInput = memo(() => {
    ...
    const { getLastRecord, updateLastRecord, addChatRecord, deleteLastRecord } =
        useChatRecordStore();
    const waveData = useRef<Uint8Array | null>();
    const drawId = useRef<number | null>(null);
            ...
            onMessage: (action: string, data: Uint8Array) => {
                ...
                switch (recvAction) {
                    ...
                    case WS_RECV_ACTION_TYPE.ENGINE_PARTIAL_OUTPUT:
                        ...
                        if (
                            lastChatRecord &&
                            lastChatRecord.role == CHAT_ROLE.AI
                        ) {
                            abort();
                            addChatRecord({
                                role: CHAT_ROLE.HUMAN,
            ...
            (chunk: Uint8Array) => {
                try {
                    if (asrWsClient.isConnected && engineReady.current) {
                        asrWsClient.sendMessage(
                            WS_SEND_ACTION_TYPE.ENGINE_PARTIAL_INPUT,
                            chunk,
                        );
                    }
                } catch (error: any) {
